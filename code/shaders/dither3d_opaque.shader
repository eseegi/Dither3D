 ...
 * 
 * This shader applies surface-stable fractal dithering to opaque surfaces.
 * 
...
cbuffer MaterialParams : register(b0)
{
    ...
    float _Cutoff;              // Alpha cutoff (for cutout mode)
};
...
struct VertexInput
{
    ...
    float2 uv0 : TEXCOORD0;     // Main UV
    float2 uv1 : TEXCOORD1;     // Lightmap UV (optional)
...
PixelInput VertexShader(VertexInput input)
{
    ...
    // NOTE: In s&box, replace UNITY_MATRIX_MVP with s&box's transformation system
    // This might be through built-in variables or a different approach
...
float4 PixelShader(PixelInput input) : SV_Target
{
    ...
    // Apply dithering
    // Use the color dithering function with appropriate mode defined at compile time
    float4 ditheredColor = GetDither3DColor(input.uv, input.screenPos, float4(finalColor, alpha));
    
    // For opaque shader, we output the dithered color
    // The PBR lighting would normally be applied here, but for dithering
    // we're replacing the final color with the dithered version
    
    return float4(ditheredColor.rgb, alpha);
}

// Fallback techniques for different rendering paths
technique11 RenderOpaque
{
    pass P0
    {
